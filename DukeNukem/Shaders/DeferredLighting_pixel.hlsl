...
float ComputeShadowFactorForPointLight(float3 position, float3 lightPosition, float sceneDepth, float range, float2 offset)
{
	...
	float closestDepth = light0ShadowMap.Sample(light0ShadowMapSampler, fragToLight + (float3(offset, 1.0) * (1.0f / 512.0f))).r;
	closestDepth *= range;
...
float3 DirectIllumination(float3 pos, float3 norm, float3 lightposition, float lightradius, float3 lightcolor, float inSpec, float3x3 TBN)
{
	...
	return  (lightcolor *att * (diffuseFactor + specFactor));
}
	...
	float shadowFactor = 0;
	{
		...
		for (y = -1.5; y <= 1.5; y += 1.0)
			for (x = -1.5; x <= 1.5; x += 1.0)
				shadowFactor += ComputeShadowFactorForPointLight(position.xyz, lightposition_and_range.xyz, linearDepth, lightposition_and_range.w, float2(x, y));
