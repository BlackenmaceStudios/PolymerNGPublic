...
GetRotation ( short tSpriteNum, long viewx, long viewy )
{
    ...
    SPRITEp tsp = ( SPRITEp ) &tsprite[tSpriteNum];
...
{
    SPRITEp newp = ( SPRITEp ) &tsprite[spritesortcnt];
    USERp tu = User[tsp->owner];
    long ground_dist = 0;
    long view_dist = 0;
    long loz;
    ...
    tsp->sectnum = sectnum;
    memcpy ( newp, tsp, sizeof ( SPRITE ) );
    // shadow is ALWAYS draw last - status is priority
    newp->statnum = MAXSTATUS;
    newp->sectnum = sectnum;
    
    if ( ( tsp->yrepeat >> 2 ) > 4 )
    {
        yrepeat = ( tsp->yrepeat >> 2 ) - ( SPRITEp_SIZE_Y ( tsp ) / 64 ) * 2;
        xrepeat = newp->xrepeat;
    }
    
    else
    {
        yrepeat = newp->yrepeat;
        xrepeat = newp->xrepeat;
    }
    
    newp->shade = 127;
    SET ( newp->cstat, CSTAT_SPRITE_TRANSLUCENT );
    loz = tu->loz;
    ...
    newp->xrepeat = xrepeat;
    newp->yrepeat = yrepeat;
...
DoMotionBlur ( SPRITEp tsp )
{
    ...
    {
        newp = ( SPRITEp ) &tsprite[spritesortcnt];
        memcpy ( newp, tsp, sizeof ( SPRITE ) );
        SET ( newp->cstat, CSTAT_SPRITE_TRANSLUCENT | CSTAT_SPRITE_TRANS_FLIP );
        newp->x += dx;
        newp->y += dy;
        dx += nx;
        dy += ny;
        ...
        TRAVERSE_SPRITE_STAT ( headspritestat[STAT_WARP_COPY_SPRITE2], sn2, nsn2 )
        {
            ...
            if ( sp->lotag == match )
            {
                ...
                TRAVERSE_SPRITE_SECT ( headspritesect[sect1], spnum, next_spnum )
                {
                    ...
                    newp = ( SPRITEp ) &tsprite[spritesortcnt];
                    memcpy ( newp, &sprite[spnum], sizeof ( SPRITE ) );
                    spritesortcnt++;
                    newp->owner = spnum;
                    newp->statnum = 0;
                    xoff = sp1->x - newp->x;
...
analyzesprites ( long viewx, long viewy, long viewz, BOOL mirror )
{
    ...
    for ( tSpriteNum = spritesortcnt - 1; tSpriteNum >= 0; tSpriteNum-- )
    {
        ...
        tsp = ( SPRITEp ) &tsprite[tSpriteNum];
        tu = User[SpriteNum];
		//initprintf("spritesortcnt = %d\n", tSpriteNum);
...
#define DART_REPEAT 16
            ...
            if ( tsp->pal != sector[tsp->sectnum].floorpal )
            {
                ...
                else
                {
                    ...
                    // sprite does not take on the newp pal if sector flag is set
...
get_tsprite ( short SpriteNum )
{
    ...
    for ( tSpriteNum = spritesortcnt - 1; tSpriteNum >= 0; tSpriteNum-- )
    {
        ...
        {
            return ( SPRITEp ) ( &tsprite[tSpriteNum] );
        }
    }
...
int CopySprite ( SPRITEp tsp, short newsector )
{
    short newp;
    SPRITEp sp;
    newp = COVERinsertsprite ( newsector, STAT_FAF_COPY );
    sp = &sprite[newp];
    sp->x = tsp->x;
    sp->y = tsp->y;
    sp->z = tsp->z;
    ...
    TRAVERSE_SPRITE_STAT ( headspritestat[STAT_CEILING_FLOOR_PIC_OVERRIDE], si, snexti )
    {
        TRAVERSE_SPRITE_SECT ( headspritesect[sprite[si].sectnum], i, nexti )
        {
            if ( User[i] )
            {
                ...
                sp = &sprite[i];
                u = User[i];
                newp = ConnectCopySprite ( sp );
                
                if ( newp >= 0 )
                {
                    np = &sprite[newp];
                    // spawn a user
                    User[newp] = nu = ( USERp ) CallocMem ( sizeof ( USER ), 1 );
                    ASSERT ( nu != NULL );
                    nu->xchange = -989898;
...
#define TEN_PIC 5109
    ...
    VisViewChange ( camerapp, ( long * ) &g_visibility );
    tz = tz + quake_z;
    tx = tx + quake_x;
    ty = ty + quake_y;
