...
#define WALL_FLOOR_SHRAP 4097
    ...
    switch ( pu->ID )
    {
                ...
                case SPAWN_SPOT:
                    ...
                    AutoShrap:
                        ...
                        switch ( shrap_type )
                        {
                            ...
                            case SHRAP_GLASS:
                                PlaySound ( DIGI_BREAKGLASS, &parent->x, &parent->y, &parent->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
                                p = GlassShrap;
                            ...
                            case SHRAP_GENERIC:
                            case SHRAP_STONE:
                                PlaySound ( DIGI_BREAKSTONES, &parent->x, &parent->y, &parent->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
                                p = StoneShrap;
                            ...
                            case SHRAP_WOOD:
                                PlaySound ( DIGI_BREAKINGWOOD, &parent->x, &parent->y, &parent->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
                                p = WoodShrap;
                            ...
                            case SHRAP_GIBS:
                                PlaySound ( DIGI_GIBS1, &parent->x, &parent->y, &parent->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
                                p = SmallGore;
                            ...
                            case SHRAP_METAL:
                                PlaySound ( DIGI_BREAKMETAL, &parent->x, &parent->y, &parent->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
                                p = MetalShrap;
                            ...
                            case SHRAP_COIN:
                                PlaySound ( DIGI_COINS, &parent->x, &parent->y, &parent->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
                                p = CoinShrap;
                            ...
                            case SHRAP_MARBELS:
                                {
                                    ...
                                    #endif
                                    //PlaySound(DIGI_BREAKMARBELS,&parent->x,&parent->y,&parent->z,Voc3D_Flags(v3df_dontpan|v3df_doppler));
                                    p = Marbels;
        ...
        case BREAK_BARREL:
            PlaySound ( DIGI_BREAKDEBRIS, &parent->x, &parent->y, &parent->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
            p = WoodShrap;
...
SpawnBlood ( short SpriteNum, short Weapon, short hitang, long hitx, long hity, long hitz )
{
    ...
    for ( ; p->state; p++ )
    {
        ...
        for ( i = 0; i < p->num; i++ )
        {
            newp = SpawnSprite ( STAT_SKIP4, p->id, p->state, sp->sectnum,
                ...
                case NINJA_RUN_R0: //sword
                    if ( wu->PlayerP )
                    {
                        ...
                        if ( wu->WeaponNum == WPN_FIST && STD_RANDOM_RANGE ( 1000 ) > 500 && pp == Player + myconnectindex )
                        {
                            ...
                            if ( choosesnd == 0 )
                            {
                                PlayerSound ( DIGI_KUNGFU, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else if ( choosesnd == 1 )
                            {
                                PlayerSound ( DIGI_PAYINGATTENTION, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else if ( choosesnd == 2 )
                            {
                                PlayerSound ( DIGI_EATTHIS, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else if ( choosesnd == 3 )
                            {
                                PlayerSound ( DIGI_TAUNTAI4, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else if ( choosesnd == 4 )
                            {
                                PlayerSound ( DIGI_TAUNTAI5, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else if ( choosesnd == 5 )
                            {
                                PlayerSound ( DIGI_HOWYOULIKEMOVE, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            //PlayerSound(TauntAIVocs[choosesnd],&pp->posx,
                            //    &pp->posy,&pp->posy,Voc3D_Flags(v3df_dontpan|v3df_follow),pp);
                        }
                        ...
                        else if ( wu->WeaponNum == WPN_SWORD && STD_RANDOM_RANGE ( 1000 ) > 500 && pp == Player + myconnectindex )
                        {
                            ...
                            if ( choose_snd > 750 )
                            {
                                PlayerSound ( DIGI_SWORDGOTU1, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else if ( choose_snd > 575 )
                            {
                                PlayerSound ( DIGI_SWORDGOTU2, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else if ( choose_snd > 250 )
                            {
                                PlayerSound ( DIGI_SWORDGOTU3, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                            
                            else
                            {
                                PlayerSound ( DIGI_CANBEONLYONE, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                            }
                        }
...
ActorHealth ( short SpriteNum, short amt )
{
    ...
    {
        PlaySpriteSound ( SpriteNum, attr_pain, Voc3D_Flags ( v3df_follow | v3df_dontpan ) );
    }
...
DoDamage ( short SpriteNum, short Weapon )
{
    ...
    switch ( wu->ID )
    {
        case NINJA_RUN_R0:
            ...
            else
            {
                ...
                PlaySound ( DIGI_CGTHIGHBONE, &wp->x, &wp->y, &wp->z, Voc3D_Flags ( v3df_follow | v3df_dontpan ) );
            }
            ...
            else if ( u->PlayerP )
            {
                ...
                if ( u->PlayerP->Armor )
                {
                    PlaySound ( DIGI_ARMORHIT, &u->PlayerP->posx, &u->PlayerP->posy, &u->PlayerP->posz, Voc3D_Flags ( v3df_dontpan | v3df_follow | v3df_doppler ) );
                }
            }
        ...
        case HORNET_RUN_R0:
            PlaySound ( DIGI_HORNETSTING, &sp->x, &sp->y, &sp->z, Voc3D_Flags ( v3df_follow | v3df_dontpan ) );
                ...
                if ( PlayerTakeDamage ( u->PlayerP, Weapon ) )
                {
                    ...
                    if ( PlayerCheckDeath ( u->PlayerP, Weapon ) )
                    {
                        PlaySound ( DIGI_RIPPERHEARTOUT, &u->PlayerP->posx, &u->PlayerP->posy,
                                    &u->PlayerP->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
        ...
        case RADIATION_CLOUD:
            ...
            else if ( u->PlayerP )
            {
                if ( PlayerTakeDamage ( u->PlayerP, Weapon ) )
                {
                    ...
                    PlayerSound ( DIGI_GASHURT, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_follow | v3df_doppler ), pp );
        ...
        case CALTROPS:
            ...
            else if ( u->PlayerP )
            {
                if ( PlayerTakeDamage ( u->PlayerP, Weapon ) )
                {
                    ...
                    {
                        PlayerSound ( DIGI_STEPONCALTROPS, ( long * ) &sp->x, ( long * ) &sp->y, ( long * ) &sp->z, Voc3D_Flags ( v3df_follow | v3df_dontpan ), u->PlayerP );
                    }
    ...
    if ( u && u->PlayerP )
    {
        ...
        {
            PlayerSound ( DIGI_SONOFABITCH, ( long * ) &sp->x, ( long * ) &sp->y, ( long * ) &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_follow ), u->PlayerP );
        }
        
        else if ( damage <= -40 && RANDOM_RANGE ( 1000 ) > 700 )
        {
            PlayerSound ( DIGI_PAINFORWEAK, ( long * ) &sp->x, ( long * ) &sp->y, ( long * ) &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_follow ), u->PlayerP );
        }
        
        else if ( damage <= -10 )
        {
            PlayerSound ( PlayerPainVocs[RANDOM_RANGE ( MAX_PAIN )], ( long * ) &sp->x, ( long * ) &sp->y, ( long * ) &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_follow ), u->PlayerP );
        }
    }
...
DoGrenade ( SHORT Weapon )
{
    ...
    if ( !TEST ( u->Flags, SPR_BOUNCE | SPR_UNDERWATER ) )
    {
        ...
        USERp nu;
        short newp;
        newp = SpawnSprite ( STAT_MISSILE, PUFF, s_Puff, sp->sectnum,
                             sp->x, sp->y, sp->z, sp->ang, 100 );
        np = &sprite[newp];
        nu = User[newp];
        SetOwner ( Weapon, newp );
        np->shade = -40;
        np->xrepeat = 40;
        np->yrepeat = 40;
        nu->ox = u->ox;
                    ...
                    if ( TEST ( hsp->extra, SPRX_PLAYER_OR_ENEMY ) )
                    {
                        ...
                        if ( sp->owner >= 0 )
                        {
                            ...
                            if ( uo && uo->PlayerP )
                            {
                                ...
                                {
                                    PlayerSound ( DIGI_STICKYGOTU1, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                                }
                                
                                else if ( RANDOM_RANGE ( 1000 ) > 800 )
                                {
                                    PlayerSound ( DIGI_STICKYGOTU2, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                                }
                                
                                else if ( RANDOM_RANGE ( 1000 ) > 800 )
                                {
                                    PlayerSound ( DIGI_STICKYGOTU3, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                                }
                                
                                else if ( RANDOM_RANGE ( 1000 ) > 800 )
                                {
                                    PlayerSound ( DIGI_STICKYGOTU4, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
                                }
                            }
                        }
                    }
...
DoLaser ( SHORT Weapon )
{
    ...
    while ( TRUE )
    {
        ...
        if ( spawn_count < 256 )
        {
            newp = SpawnSprite ( STAT_MISSILE, PUFF, s_LaserPuff, sp->sectnum,
...
DoRail ( SHORT Weapon )
{
    ...
    while ( TRUE )
    {
        ...
        if ( spawn_count < 128 )
        {
            newp = SpawnSprite ( STAT_MISSILE, PUFF, &s_RailPuff[0][0], sp->sectnum,
            ...
            nu->RotNum = 5;
            NewStateGroup ( newp, sg_RailPuff );
            np->shade = -40;
            np->xrepeat = 10;
            np->yrepeat = 10;
            nu->ox = u->ox;
...
SpawnFireballFlames ( SHORT SpriteNum, SHORT enemy )
{
    ...
    newp = SpawnSprite ( STAT_MISSILE, FIREBALL_FLAMES, s_FireballFlames, sp->sectnum,
                         sp->x, sp->y, sp->z, sp->ang, 0 );
    np = &sprite[newp];
    nu = User[newp];
    ...
    PlaySound ( DIGI_FIRE1, &np->x, &np->y, &np->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    Set3DSoundOwner ( newp );
    return ( newp );
}
...
SpawnBreakFlames ( SHORT SpriteNum, SHORT enemy )
{
    ...
    USERp nu;
    short newp;
    newp = SpawnSprite ( STAT_MISSILE, FIREBALL_FLAMES + 1, s_BreakFlames, sp->sectnum,
                         sp->x, sp->y, sp->z, sp->ang, 0 );
    np = &sprite[newp];
    nu = User[newp];
    np->hitag = LUMINOUS; //Always full brightness
...
SpawnBreakStaticFlames ( SHORT SpriteNum )
{
    ...
    USERp nu;
    short newp;
    newp = SpawnSprite ( STAT_STATIC_FIRE, FIREBALL_FLAMES, NULL, sp->sectnum,
                         sp->x, sp->y, sp->z, sp->ang, 0 );
    np = &sprite[newp];
    nu = User[newp];
...
SpawnNuclearExp ( SHORT Weapon )
{
    ...
    PlaySound ( DIGI_NUCLEAREXP, &sp->x, &sp->y, &sp->z, ( Voc3D_Flags ) ( v3df_dontpan | v3df_doppler ) );
    ...
    if ( sp->owner )
    {
        ...
        if ( rnd_rng > 990 )
        {
            PlayerSound ( DIGI_LIKEHIROSHIMA, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
        }
        
        else if ( rnd_rng > 980 )
        {
            PlayerSound ( DIGI_LIKENAGASAKI, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
        }
        
        else if ( rnd_rng > 970 )
        {
            PlayerSound ( DIGI_LIKEPEARL, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
        }
    }
...
SpawnGrenadeExp ( SHORT Weapon )
{
    ...
    if ( RANDOM_RANGE ( 1000 ) > 990 )
    {
        if ( sp->owner >= 0 && User[sp->owner] && User[sp->owner]->PlayerP )
        {
            ...
            PlayerSound ( DIGI_LIKEFIREWORKS, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
        }
    }
...
DoFindGround ( SHORT SpriteNum )
{
    ...
    save_cstat = sp->cstat;
    RESET ( sp->cstat, CSTAT_SPRITE_BLOCK | CSTAT_SPRITE_BLOCK_HITSCAN );
    FAFgetzrange ( sp->x, sp->y, sp->z, sp->sectnum, ( LONGp ) &u->hiz, &ceilhit, ( LONGp ) &u->loz, &florhit, ( ( ( long ) sp->clipdist ) << 2 ) - GETZRANGE_CLIP_ADJ, CLIPMASK_PLAYER );
    sp->cstat = save_cstat;
...
DoFindGroundPoint ( SHORT SpriteNum )
{
    ...
    save_cstat = sp->cstat;
    RESET ( sp->cstat, CSTAT_SPRITE_BLOCK | CSTAT_SPRITE_BLOCK_HITSCAN );
    FAFgetzrangepoint ( sp->x, sp->y, sp->z, sp->sectnum, ( LONGp ) &u->hiz, &ceilhit, ( LONGp ) &u->loz, &florhit );
    sp->cstat = save_cstat;
    ASSERT ( TEST ( florhit, HIT_SPRITE | HIT_SECTOR ) );
...
DoBloodWorm ( SHORT Weapon )
{
    ...
    USERp u = User[Weapon];
    int offset;
    int move_ground_missile ( short spritenum, long xchange, long ychange, long zchange, long ceildist, long flordist, ULONG cliptype, long numtics );
    short ang;
...
DoMirv ( SHORT Weapon )
{
    ...
    if ( !u->Counter )
    {
        ...
        for ( i = 0; i < 2; i++ )
        {
            newp = SpawnSprite ( STAT_MISSILE, MIRV_METEOR, &sg_MirvMeteor[0][0], sp->sectnum,
                                 sp->x, sp->y, sp->z, NORM_ANGLE ( sp->ang + angs[i] ), 800 );
            np = &sprite[newp];
            nu = User[newp];
            nu->RotNum = 5;
            NewStateGroup ( newp, &sg_MirvMeteor[0] );
...
InitSerpRing ( short SpriteNum )
{
    SPRITEp sp = User[SpriteNum]->SpriteP, np;
    USERp u = User[SpriteNum], nu;
    short ang, ang_diff, ang_start, ang_finish, missiles, newp;
    short max_missiles;
#define SERP_RING_DIST 2800 // Was 3500
    ...
    for ( missiles = 0, ang = ang_start; missiles < max_missiles; ang += ang_diff, missiles++ )
    {
        newp = SpawnSprite ( STAT_SKIP4, SKULL_SERP, &s_SkullRing[0][0], sp->sectnum, sp->x, sp->y, sp->z, ang, 0 );
        np = &sprite[newp];
        nu = User[newp];
        np->xvel = 500;
        //np->owner = SpriteNum;
        SetOwner ( SpriteNum, newp );
        np->shade = -20;
        np->xrepeat = 64;
        np->yrepeat = 64;
        ...
        EnemyDefaults ( newp, NULL, NULL );
        change_sprite_stat ( newp, STAT_SKIP4 );
...
#define NUM_SERP_RING 2
    ...
    for ( i = 0; i < NUM_SERP_RING; i++ )
    {
        ...
        for ( missiles = 0, ang = ang_start; missiles < max_missiles; ang += ang_diff, missiles++ )
        {
            newp = SpawnSprite ( STAT_MISSILE_SKIP4, SKULL_SERP, &s_SkullRing[0][0], sp->sectnum, sp->x, sp->y, sp->z, ang, 0 );
            np = &sprite[newp];
            nu = User[newp];
            //np->owner = SpriteNum;
            SetOwner ( SpriteNum, newp );
            ...
            EnemyDefaults ( newp, NULL, NULL );
            RESET ( np->extra, SPRX_PLAYER_OR_ENEMY );
            change_sprite_stat ( newp, STAT_MISSILE_SKIP4 );
            np->clipdist = ( 128 + 64 ) >> 2;
            SET ( nu->Flags, SPR_XFLIP_TOGGLE );
            SET ( np->cstat, CSTAT_SPRITE_YCENTER );
            nu->Radius = 400;
...
InitSwordAttack ( PLAYERp pp )
{
    ...
    long dist;
    short reach, face;
    PlaySound ( DIGI_SWORDSWOOSH, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
...
InitFistAttack ( PLAYERp pp )
{
    ...
    long dist;
    short reach, face;
    PlaySound ( DIGI_STAR, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
...
InitSumoSkull ( short SpriteNum )
{
    ...
    PlaySound ( DIGI_SERPSUMMONHEADS, &sp->x, &sp->y, &sp->z, v3df_none );
    newp = SpawnSprite ( STAT_ENEMY, SKULL_R0, &s_SkullWait[0][0], sp->sectnum, sp->x, sp->y, SPRITEp_MID ( sp ), sp->ang, 0 );
    np = &sprite[newp];
    nu = User[newp];
    np->xvel = 500;
    SetOwner ( SpriteNum, newp );
...
InitSumoStompAttack ( short SpriteNum )
{
    ...
    long dist;
    short reach, face;
    PlaySound ( DIGI_30MMEXPLODE, &sp->x, &sp->y, &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
...
InitMiniSumoClap ( short SpriteNum )
{
    ...
    if ( dist < CLOSE_RANGE_DIST_FUDGE ( u->tgt_sp, sp, 1000 ) )
    {
        if ( SpriteOverlapZ ( SpriteNum, u->tgt_sp - sprite, Z ( 20 ) ) )
        {
            ...
            {
                PlaySound ( DIGI_CGTHIGHBONE, &sp->x, &sp->y, &sp->z, Voc3D_Flags ( v3df_follow | v3df_dontpan ) );
...
InitShotgun ( PLAYERp pp )
{
    ...
    PlaySound ( DIGI_RIOTFIRE2, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
...
InitLaser ( PLAYERp pp )
{
    ...
    PlayerUpdateAmmo ( pp, u->WeaponNum, -1 );
    PlaySound ( DIGI_RIOTFIRE, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    nx = pp->posx;
    ny = pp->posy;
    nz = pp->posz + pp->bob_z + Z ( 8 );
    // Spawn a shot
...
InitRail ( PLAYERp pp )
{
    ...
    DoPlayerBeginRecoil ( pp, RAIL_RECOIL_AMT );
    PlayerUpdateAmmo ( pp, u->WeaponNum, -1 );
    PlaySound ( DIGI_RAILFIRE, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    // Make sprite shade brighter
    u->Vis = 128;
    nx = pp->posx;
    ny = pp->posy;
...
InitZillaRail ( short SpriteNum )
{
    ...
    PlaySound ( DIGI_RAILFIRE, &sp->x, &sp->y, &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    // Make sprite shade brighter
    u->Vis = 128;
    nx = sp->x;
    ny = sp->y;
...
InitBunnyRocket ( PLAYERp pp )
{
    ...
    DoPlayerBeginRecoil ( pp, ROCKET_RECOIL_AMT );
    PlayerUpdateAmmo ( pp, u->WeaponNum, -1 );
    PlaySound ( DIGI_BUNNYATTACK, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    nx = pp->posx;
    ny = pp->posy;
    // Spawn a shot
    // Inserting and setting up variables
...
InitEnemyNuke ( short SpriteNum )
{
    ...
    short oclipdist;
    long zvel;
    PlaySound ( DIGI_RIOTFIRE, &sp->x, &sp->y, &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    // Make sprite shade brighter
    u->Vis = 128;
    nx = sp->x;
    ny = sp->y;
...
InitSerpSpell ( short SpriteNum )
{
    ...
    {
        sp->ang = getangle ( u->tgt_sp->x - sp->x, u->tgt_sp->y - sp->y );
        newp = SpawnSprite ( STAT_MISSILE, SERP_METEOR, &sg_SerpMeteor[0][0], sp->sectnum,
                             sp->x, sp->y, sp->z, sp->ang, 1500 );
        np = &sprite[newp];
        nu = User[newp];
        np->z = SPRITEp_TOS ( sp );
        nu->RotNum = 5;
        NewStateGroup ( newp, &sg_SerpMeteor[0] );
        nu->StateEnd = s_MirvMeteorExp;
        //np->owner = SpriteNum;
        SetOwner ( SpriteNum, newp );
        np->shade = -40;
        ...
        nu->ychange = MOVEy ( np->xvel, np->ang );
        nu->zchange = np->zvel;
        MissileSetPos ( newp, DoMirvMissile, 400 );
        sp->clipdist = oclipdist;
...
InitCoolgFire ( short SpriteNum )
{
    ...
    PlaySound ( DIGI_MAGIC1, &wp->x, &wp->y, &wp->z, Voc3D_Flags ( v3df_follow | v3df_doppler ) );
    // find the distance to the target (player)
    dist = Distance ( nx, ny, u->tgt_sp->x, u->tgt_sp->y );
...
BulletHitSprite ( SPRITEp sp, short hitsprite, short hitsect, short hitwall, long hitx, long hity, long hitz, short ID )
{
    ...
    if ( TEST ( hsp->extra, SPRX_PLAYER_OR_ENEMY ) )
    {
        ...
        newp = SpawnSprite ( STAT_MISSILE, id, s_UziSmoke, 0, hitx, hity, hitz, sp->ang, 0 );
        wp = &sprite[newp];
        wu = User[newp];
        wp->shade = -40;
        ...
        SetOwner ( sp - sprite, newp );
        wp->ang = sp->ang;
        setsprite ( newp, hitx, hity, hitz );
        SET ( wp->cstat, CSTAT_SPRITE_YCENTER );
        RESET ( wp->cstat, CSTAT_SPRITE_BLOCK | CSTAT_SPRITE_BLOCK_HITSCAN );
        ...
        DoHitscanDamage ( newp, hitsprite );
        return ( TRUE );
    }
...
HitscanSpriteAdjust ( short SpriteNum, short hitwall )
{
    ...
    // must have this
    sectnum = sp->sectnum;
    clipmove ( ( long * ) &sp->x, ( long * ) &sp->y, ( long * ) &sp->z, &sectnum, xvect, yvect,
...
#define UZIFIRE_WAIT 20
    ...
    if ( FireSnd )
    {
        PlaySound ( DIGI_UZIFIRE, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    }
...
InitEMP ( PLAYERp pp )
{
    ...
    if ( hitsprite >= 0 )
    {
        ...
        if ( TEST ( hsp->extra, SPRX_PLAYER_OR_ENEMY ) )
        {
            ...
            if ( RANDOM_RANGE ( 1000 ) > 500 )
            {
                PlayerSound ( DIGI_YOULOOKSTUPID, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_follow | v3df_dontpan ), pp );
            }
        }
...
InitTankShell ( short SpriteNum, PLAYERp pp )
{
    ...
    if ( !SW_SHAREWARE )
    {
        PlaySound ( DIGI_CANNON, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    }
...
InitSobjMachineGun ( short SpriteNum, PLAYERp pp )
{
    ...
    //sound = (++sound)&1;
    //if (sound == 0)
    PlaySound ( DIGI_BOATFIRE, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    nx = sp->x;
    ny = sp->y;
    daz = nz = sp->z;
    nsect = sp->sectnum;
...
InitTurretMgun ( SECTOR_OBJECTp sop )
{
    ...
    SPRITEp sp, ep;
    long xvect, yvect, zvect;
    PlaySound ( DIGI_BOATFIRE, &sop->xmid, &sop->ymid, &sop->zmid, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
...
InitGrenade ( PLAYERp pp )
{
    ...
    PlayerUpdateAmmo ( pp, u->WeaponNum, -1 );
    PlaySound ( DIGI_30MMFIRE, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    // Make sprite shade brighter
    u->Vis = 128;
    nx = pp->posx;
    ny = pp->posy;
...
InitSpriteGrenade ( short SpriteNum )
{
    ...
    short oclipdist;
    long dist;
    PlaySound ( DIGI_30MMFIRE, &sp->x, &sp->y, &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    nx = sp->x;
    ny = sp->y;
    nz = sp->z - Z ( 40 );
    // Spawn a shot
...
InitMine ( PLAYERp pp )
{
    ...
    long dot;
    PlayerUpdateAmmo ( pp, u->WeaponNum, -1 );
    PlaySound ( DIGI_MINETHROW, &pp->posx, &pp->posy, &pp->posz, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    nx = pp->posx;
    ny = pp->posy;
    nz = pp->posz + pp->bob_z + Z ( 8 );
    // Spawn a shot
...
InitEnemyMine ( short SpriteNum )
{
    ...
    long dx, dy, dz;
    long tvel;
    PlaySound ( DIGI_MINETHROW, &sp->x, &sp->y, &sp->z, Voc3D_Flags ( v3df_dontpan | v3df_doppler ) );
    nx = sp->x;
    ny = sp->y;
    nz = sp->z - Z ( 40 );
    // Spawn a shot
...
WarpToUnderwater ( short *sectnum, long *x, long *y, long *z )
{
    ...
    sx = over_sp->x - *x;
    sy = over_sp->y - *y;
    // update to the newp x y position
    *x = under_sp->x - sx;
    *y = under_sp->y - sy;
...
SpawnVehicleSmoke ( short SpriteNum )
{
    ...
    newp = SpawnSprite ( STAT_MISSILE, PUFF, s_VehicleSmoke, sp->sectnum,
...
SpawnSmokePuff ( short SpriteNum )
{
    ...
    USERp u = User[SpriteNum], nu;
    short newp;
    newp = SpawnSprite ( STAT_MISSILE, PUFF, s_WaterSmoke, sp->sectnum,
                         sp->x, sp->y, sp->z - RANDOM_P2 ( Z ( 8 ) ), sp->ang, 0 );
    np = &sprite[newp];
    nu = User[newp];
    nu->WaitTics = 1 * 120;
...
int QueueStar ( short SpriteNum )
{
    ...
    {
        // move old star to newp stars place
...
int QueueHole ( short ang, short hitsect, short hitwall, long hitx, long hity, long hitz )
{
    ...
    sectnum = sp->sectnum;
    clipmoveboxtracenum = 1;
    clipmove ( ( long * ) &sp->x, ( long * ) &sp->y, ( long * ) &sp->z, &sectnum, nx, ny,
...
int QueueGeneric ( short SpriteNum, short pic )
{
    ...
    {
        // move old sprite to newp sprite's place
