...
void SoundCallBack ( uint32_t num );
char *LoadMidi ( char * );
...
// CTW MODIFICATION END
{
    ...
    {
        ASSERT ( -num < 11 );
        lumplockbyte[- ( long ) num] = lumplockbyte[- ( long ) num] - 1;
        return;
    }
...
int PlaySong ( char *song_file_name )
{
    ...
    int32_t fp = S_OpenAudio ( song_file_name, 0, 1 );
    ...
    if ( EDUKE32_PREDICT_FALSE ( g_musicSize != MusicLen ) )
    {
        OSD_Printf ( OSD_ERROR "S_PlayMusic(): error: read %d bytes from \"%s\", expected %d\n",
                     g_musicSize, song_file_name, MusicLen );
        kclose ( fp );
        g_musicSize = 0;
        return 0;
    }
    ...
    //MONO_PRINT(ds);
    int32_t const mvol = gs.MusicVolume;
...
void LockSound ( int num )
{
        ...
        // if data is already locked
    {
        ...
        {
            // jmarshall - eval taking this out
            //DumpSounds();
            //TerminateGame();
    ...
    if ( !vp->data )
    {
        ...
        if ( vp != NULL )
        {
            ...
            */
            ...
            vp->data_length = length;
            allocache ( ( intptr_t * ) ( &vp->data ), length, ( char * ) &vp->lock );
            #if 0
            // DEBUG
...
int
PlaySound ( int num, int32_t *x, int32_t *y, int32_t *z, Voc3D_Flags flags )
{
    VOC_INFOp vp;
    VOC3D_INFOp v3p;
    int pitch = 0;
    ...
    if ( gs.Ambient && TEST ( flags, v3df_ambient ) && !TEST ( flags, v3df_nolookup ) )
    {
        ...
        flags = ( Voc3D_Flags ) ( ( int ) flags | ( int ) ambarray[num].ambient_flags ); // Add to flags if any
    ...
    if ( vp->voc_flags & vf_loop )
    {
        ...
        {
            voice = FX_PlayLoopedAuto ( ( char * ) vp->data, start, 65536,
                                        pitch, loopvol, loopvol, loopvol, priority, num, 0 );
        }
        ...
        {
            voice = FX_PlayAuto ( ( char * ) vp->data, vp->data_length, pitch, 255, 255, 255, priority, num );
        }
        ...
        else // It's a 3d sound
        {
            if ( sound_dist < 255 )
            {
                voice = FX_PlayAuto ( ( char * ) vp->data, vp->data_length, pitch, angle, 255, sound_dist, sound_dist, priority );
            }
    ...
    if ( flags & v3df_init )
    {
        v3p->flags = ( Voc3D_Flags ) ( ( int ) v3p->flags ^ ( int ) v3df_init );    // Turn init off now
    }
...
VOID PlaySoundRTS ( long rts_num )
{
    ...
    if ( *rtsptr == 'C' )
    {
        voice = FX_PlayAuto ( ( char * ) rtsptr, 0, 0, 0, 255, -rts_num, 0, 0 );
    }
...
void SoundStartup ( void )
{
    ...
    //gs.FxOn = TRUE;
    status = FX_Init ( FXDevice, NumVoices, NumChannels, MixRate, NULL );
...
void MusicStartup ( void )
{
    ...
    status = MUSIC_Init ( 0, 0 ) == MUSIC_Ok || MUSIC_Init ( 1, 0 ) == MUSIC_Ok;
...
//////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////
// Initialize newp vocs in the 3D sound queue
///////////////////////////////////////////////
VOC3D_INFOp
InitNew3DSound ( VOC3D_INFOp v3p )
{
    v3p->handle = -1;                    // Initialize handle to newp sound
    // value
    v3p->owner = -1;
...
///////////////////////////////////////////////
// Inserts newp vocs in the 3D sound queue
///////////////////////////////////////////////
VOC3D_INFOp
Insert3DSound ( void )
{
    VOC3D_INFOp vp, old;
    // Allocate memory for newp sound
    // You can allocate newp sounds as long as memory holds out.
...
void DeleteNoFollowSoundOwner ( short spritenum )
{
    ...
    while ( vp )
    {
        ...
        if ( ( vp->flags & v3df_follow ) && vp->x == ( long * ) &sp->x && vp->y == ( long * ) &sp->y )
        {
            if ( FX_SoundActive ( vp->handle ) )
            {
                FX_StopSound ( vp->handle );
...
#define MAXRNDAMB 12
    ...
    // If ambient sound is found in the array, randomly pick a newp sound
    ...
    return ( handle ); // Give back the sound, no newp one was found
}
...
StartAmbientSound ( void )
{
    ...
    {
        SPRITEp sp = &sprite[i];
        PlaySound ( ( uint16_t ) sp->lotag, ( int32_t * ) &sp->x, ( int32_t * ) &sp->y, ( int32_t * ) &sp->z, ( Voc3D_Flags ) ( v3df_ambient | v3df_init | v3df_doppler | v3df_follow ) );
...
#define DOPPLERMAX  400
                    ...
                    if ( ! ( p->flags & v3df_doppler ) && FX_SoundActive ( p->handle ) )
                    {
                        ...
                        p->doplr_delta = dist;  // Save newp distance to
                        // struct
