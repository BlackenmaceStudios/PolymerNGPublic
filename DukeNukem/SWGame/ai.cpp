...
DoActorActionDecide ( short SpriteNum )
{
    ...
    else
    {
        ...
        // pick a closeby player as the (newp) target
...
FindNewAngle ( short SpriteNum, signed char dir, long DistToMove )
{
    ...
    for ( ; *adp != -99; adp++ )
    {
        ...
        if ( ret == 0 )
        {
            // cleanly moved in newp direction without hitting something
...
/*
  ...
  pick a newp direction close to the target direction (or away from the target
