...
// x & y offset of tile
#define TILE_XOFF(picnum) ((CHAR)TEST(picanm[(picnum)].rawbits >> 8, 0xFF))
#define TILE_YOFF(picnum) ((CHAR)TEST(picanm[(picnum)].rawbits >> 16, 0xFF))

// x & y offset of current sprite tile
#define SPRITEp_XOFF(sp) ((CHAR)TEST(picanm[(sp)->picnum].rawbits >> 8, 0xFF))
#define SPRITEp_YOFF(sp) ((CHAR)TEST(picanm[(sp)->picnum].rawbits >> 16, 0xFF))
...
// newp define more readable defines
...
__forceinline BYTE CDAudio_GetVolume ( void )
{
    return 0;
}
__forceinline void CDAudio_SetVolume ( BYTE volume ) { }
__forceinline void CDAudio_Play ( BYTE track, BOOL looping ) { }
__forceinline void CDAudio_Stop ( void ) { }
__forceinline void CDAudio_Resume ( void ) { }
__forceinline static void CD_f ( void ) { }
__forceinline void CDAudio_Update ( void ) { }
__forceinline BOOL CDAudio_Playing ( void ) { return FALSE; }
__forceinline int CDAudio_Init ( void ) { return 0; }
__forceinline void CDAudio_Shutdown ( void ) { }
...
#define MAX_CLIPBOX 32
    ...
    short   sector[MAX_SO_SECTOR],     // hold the sector numbers of the sector object
            ...
            spin_ang,       // spin angle
            turn_speed,     // shift value determines how fast SO turns to match newp angle
...
int PlaySound ( int num, int32_t *x, int32_t *y, int32_t *z, Voc3D_Flags flags );
__forceinline int PlaySound ( int num, long *x, long *y, long *z, Voc3D_Flags flags )
{
    return ::PlaySound ( num, ( int32_t* ) x, ( int32_t* ) y, ( int32_t* ) z, flags );
}
...
// save player info when moving to a newp level
