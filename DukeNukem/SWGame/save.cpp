...
int SaveGame ( short save_num )
{
    ...
    for ( i = 0; i < numplayers; i++ )
    {
        ...
        TRAVERSE ( &pp->PanelSpriteList, cur, next )
        {
            ...
            ( ( PANEL_SPRITEp ) psp )->sibling = ( PANEL_SPRITEp ) PanelSpriteToNdx ( pp, ( ( PANEL_SPRITEp ) cur )->sibling );
            MWRITE ( psp, sizeof ( ORG_TILE ), 1, fil );
            saveisshot |= SaveSymDataInfo ( fil, ( ( PANEL_SPRITEp ) psp )->PlayerP );
            saveisshot |= SaveSymDataInfo ( fil, ( ( PANEL_SPRITEp ) psp )->State );
            saveisshot |= SaveSymDataInfo ( fil, ( ( PANEL_SPRITEp ) psp )->RetractState );
            saveisshot |= SaveSymDataInfo ( fil, ( ( PANEL_SPRITEp ) psp )->PresentState );
            saveisshot |= SaveSymDataInfo ( fil, ( ( PANEL_SPRITEp ) psp )->ActionState );
            saveisshot |= SaveSymDataInfo ( fil, ( ( PANEL_SPRITEp ) psp )->RestState );
            saveisshot |= SaveSymCodeInfo ( fil, ( ( PANEL_SPRITEp ) psp )->PanelSpriteFunc );
            
            for ( j = 0; j < SIZ ( ( ( PANEL_SPRITEp ) psp )->over ); j++ )
            {
                saveisshot |= SaveSymDataInfo ( fil, ( ( PANEL_SPRITEp ) psp )->over[j].State );
            }
...
int LoadGame ( short save_num )
{
    ...
    for ( i = 0; i < numplayers; i++ )
    {
        ...
        while ( TRUE )
        {
            ...
            psp = ( OrgTileP ) CallocMem ( sizeof ( PANEL_SPRITE ), 1 );
            ASSERT ( psp );
            MREAD ( psp, sizeof ( PANEL_SPRITE ), 1, fil );
            INSERT_TAIL ( &pp->PanelSpriteList, psp );
            saveisshot |= LoadSymDataInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->PlayerP );
            saveisshot |= LoadSymDataInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->State );
            saveisshot |= LoadSymDataInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->RetractState );
            saveisshot |= LoadSymDataInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->PresentState );
            saveisshot |= LoadSymDataInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->ActionState );
            saveisshot |= LoadSymDataInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->RestState );
            saveisshot |= LoadSymCodeInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->PanelSpriteFunc );
            
            if ( saveisshot ) { MCLOSE ( fil ); return -1; }
            
            for ( j = 0; j < ( long ) SIZ ( ( ( PANEL_SPRITEp ) psp )->over ); j++ )
            {
                saveisshot |= LoadSymDataInfo ( fil, ( void** ) & ( ( PANEL_SPRITEp ) psp )->over[j].State );
    ...
    while ( SpriteNum != -1 )
    {
        ...
        if ( u->WallShade )
        {
            u->WallShade = ( CHARp ) CallocMem ( u->WallCount * sizeof ( *u->WallShade ), 1 );
        ...
        if ( u->rotator )
        {
            ...
            if ( u->rotator->origx )
            {
                u->rotator->origx = ( long * ) CallocMem ( u->rotator->num_walls * sizeof ( *u->rotator->origx ), 1 );
                MREAD ( u->rotator->origx, sizeof ( *u->rotator->origx ) *u->rotator->num_walls, 1, fil );
            }
            
            if ( u->rotator->origy )
            {
                u->rotator->origy = ( long * ) CallocMem ( u->rotator->num_walls * sizeof ( *u->rotator->origy ), 1 );
    ...
    //    MREAD(&g_visibility,sizeof(visibility),1,fil);
    ...
    for ( i = 0; i < numplayers; i++ )
    {
        ...
        TRAVERSE ( &pp->PanelSpriteList, psp, next )
        {
            ...
            ( ( PANEL_SPRITEp ) psp )->sibling = PanelNdxToSprite ( pp, ( long ) ( ( PANEL_SPRITEp ) psp )->sibling );
        }
    }
