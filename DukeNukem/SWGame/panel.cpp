...
{
    PANEL_SPRITEp nsp;
    OrgTileP psp = NULL, next;
    TRAVERSE ( &pp->PanelSpriteList, psp, next )
    {
        // early out
        if ( ( ( PANEL_SPRITEp ) psp )->priority > pri )
        {
            return ( NULL );
        }
        
        if ( ( ( PANEL_SPRITEp ) psp )->x == x && ( ( PANEL_SPRITEp ) psp )->y == y && ( ( PANEL_SPRITEp ) psp )->priority == pri )
        {
            return ( ( ( PANEL_SPRITEp ) psp ) );
        }
    }
    return ( NULL );
}
...
pFindMatchingSpriteID ( PLAYERp pp, short id, long x, long y, short pri )
{
    ...
    TRAVERSE ( &pp->PanelSpriteList, psp, next )
    {
        ...
        if ( ( ( PANEL_SPRITEp ) psp )->ID == id && ( ( PANEL_SPRITEp ) psp )->x == x && ( ( PANEL_SPRITEp ) psp )->y == y && ( ( PANEL_SPRITEp ) psp )->priority == pri )
        {
            return ( ( ( PANEL_SPRITEp ) psp ) );
        }
    }
...
VOID DoPlayerChooseYell ( PLAYERp pp )
{
    ...
    {
        PlayerSound ( PlayerYellVocs[choose_snd], &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), pp );
    }
}
        ...
        if ( value < 0 )
        {
            ...
            if ( u->Health > 50 )
            {
                PlayerSound ( PlayerPainVocs[choosesnd], &pp->posx,
                              &pp->posy, &pp->posy, ( Voc3D_Flags ) ( v3df_dontpan | v3df_doppler | v3df_follow ), pp );
            }
                ...
                case WPN_HOTHEAD:
                    if ( !SW_SHAREWARE )
                    {
                                ...
                                || u->WeaponNum == WPN_NAPALM )
                        {
                            ...
                            PlaySound ( DIGI_HOTHEADSWITCH, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_dontpan | v3df_follow ) );
                        }
...
RetractCurWpn ( PLAYERp pp )
{
    ...
    if ( pp->CurWpn )
    {
        ...
        else
        {
            ...
            {
                if ( ( ( PANEL_SPRITEp ) cur )->sibling && ( ( PANEL_SPRITEp ) cur )->sibling == pp->CurWpn )
                {
                    // special case for uzi reload pieces
                    ( ( PANEL_SPRITEp ) cur )->picnum = ( ( PANEL_SPRITEp ) cur )->picndx;
                    ( ( PANEL_SPRITEp ) cur )->State = NULL;
                    ( ( PANEL_SPRITEp ) cur )->PanelSpriteFunc = SpecialUziRetractFunc;
                    ( ( PANEL_SPRITEp ) cur )->sibling = NULL;
                }
            }
        }
    }
...
InitWeaponSword ( PLAYERp pp )
{
    ...
    RetractCurWpn ( pp );
    // Set up the newp Weapon variables
    ...
    PlaySound ( DIGI_SWORD_UP, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
    ...
    if ( pp == Player + myconnectindex )
    {
        ...
        if ( rnd_num > 900 )
        {
            PlaySound ( DIGI_TAUNTAI2, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
        }
        
        else if ( rnd_num > 800 )
        {
            PlaySound ( DIGI_PLAYERYELL1, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
        }
        
        else if ( rnd_num > 700 )
        {
            PlaySound ( DIGI_PLAYERYELL2, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
        }
        
        else if ( rnd_num > 600 )
        {
            PlayerSound ( DIGI_ILIKESWORD, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), pp );
        }
    }
...
InitWeaponStar ( PLAYERp pp )
{
    ...
    pSetState ( psp, psp->PresentState );
    PlaySound ( DIGI_PULL, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
    
    if ( STD_RANDOM_RANGE ( 1000 ) > 900 && pp == Player + myconnectindex )
    {
        if ( !useDarts )
        {
            PlayerSound ( DIGI_ILIKESHURIKEN, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), pp );
        }
    }
...
pSpawnUziClip ( PANEL_SPRITEp gun )
{
    ...
    PlaySound ( DIGI_REMOVECLIP, &gun->PlayerP->posx, &gun->PlayerP->posy,
                &gun->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan | v3df_doppler | v3df_follow ) );
                
    if ( TEST ( gun->flags, PANF_XFLIP ) )
    {
        newp = pSpawnSprite ( gun->PlayerP, ps_UziClip, PRI_BACK, gun->x - UZI_CLIP_XOFF, gun->y + UZI_CLIP_YOFF );
        SET ( newp->flags, PANF_XFLIP );
        newp->ang = NORM_ANGLE ( 1024 + 256 + 22 );
        newp->ang = NORM_ANGLE ( newp->ang + 512 );
    }
    ...
    newp->vel = 1050;
    SET ( newp->flags, PANF_WEAPON_SPRITE );
    ...
    if ( TEST ( nclip->flags, PANF_XFLIP ) )
    {
        if ( nclip->x < gun->x )
        {
            PlaySound ( DIGI_REPLACECLIP, &nclip->PlayerP->posx, &nclip->PlayerP->posy,
                        &nclip->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan | v3df_doppler ) );
...
pUziDoneReload ( PANEL_SPRITEp psp )
{
    ...
    if ( TEST ( psp->flags, PANF_PRIMARY ) && pp->WpnUziType == 3 )
    {
        ...
        newp = InitWeaponUziSecondaryReload ( psp );
        pp->Wpn[WPN_UZI] = newp;
        pp->CurWpn = newp;
...
InitWeaponUzi2 ( PANEL_SPRITEp uzi_orig )
{
    ...
    newp = pSpawnSprite ( pp, ps_PresentUzi2, PRI_MID, 160 - UZI_XOFF, UZI_YOFF );
    uzi_orig->sibling = newp;
    newp->y += tilesiz[newp->picndx].y;
    // Set up the newp Weapon variables
    SET ( newp->flags, PANF_WEAPON_SPRITE );
    newp->ActionState = &ps_FireUzi2[1];
    newp->RetractState = ps_RetractUzi2;
    newp->PresentState = ps_PresentUzi2;
    newp->RestState = ps_FireUzi2;
    pSetState ( newp, newp->PresentState );
    newp->sibling = uzi_orig;
    SET ( newp->flags, PANF_SECONDARY );
    pUziOverlays ( newp, CHAMBER_REST );
    return ( newp );
}
...
InitWeaponUziSecondaryReload ( PANEL_SPRITEp uzi_orig )
{
    PANEL_SPRITEp newp;
    PLAYERp pp = uzi_orig->PlayerP;
    newp = pSpawnSprite ( pp, ps_PresentUzi, PRI_MID, 160 - UZI_XOFF, UZI_YOFF );
    newp->y += tilesiz[newp->picndx].y;
    SET ( newp->flags, PANF_XFLIP );
    // Set up the newp Weapon variables
    SET ( newp->flags, PANF_WEAPON_SPRITE );
    newp->ActionState = ps_UziEject;
    newp->RetractState = ps_RetractUzi;
    newp->PresentState = ps_PresentUzi;
    newp->RestState = ps_UziEject;
    // pSetState(newp, newp->PresentState);
    pSetState ( newp, ps_PresentUziReload );
    newp->sibling = uzi_orig;
    SET ( newp->flags, PANF_SECONDARY | PANF_RELOAD );
    return ( newp );
}
...
pShotgunReloadDown ( PANEL_SPRITEp psp )
{
    ...
    if ( psp->y >= SHOTGUN_YOFF + ( tilesiz[picnum].y / 2 ) )
    {
        PlaySound ( DIGI_ROCKET_UP, &psp->PlayerP->posx, &psp->PlayerP->posy,
                    &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan | v3df_doppler ) );
...
InitWeaponRail ( PLAYERp pp )
{
    ...
    pSetState ( psp, psp->PresentState );
    PlaySound ( DIGI_RAIL_UP, &pp->posx, &pp->posy, &pp->posz, v3df_follow );
    railvochandle = PlaySound ( DIGI_RAILREADY, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
...
InitWeaponHothead ( PLAYERp pp )
{
    ...
    PlaySound ( DIGI_GRDALERT, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
}
        ...
        if ( pp == Player + myconnectindex )
        {
            ...
            if ( choose_voc > 600 )
                PlayerSound ( DIGI_TAUNTAI2, &psp->PlayerP->posx,
                              &psp->PlayerP->posy, &psp->PlayerP->posy, ( Voc3D_Flags ) ( v3df_dontpan | v3df_follow ), psp->PlayerP );
...
pMicroFire ( PANEL_SPRITEp psp )
{
    ...
    switch ( psp->PlayerP->WpnRocketType )
    {
        ...
        case 2:
            PlaySound ( DIGI_WARNING, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_dontpan | v3df_follow ) );
...
pMicroStandBy ( PANEL_SPRITEp psp )
{
    ...
    pMicroOverlays ( psp );
    pp->nukevochandle =
        PlaySound ( DIGI_NUKESTDBY, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
}
...
pMicroCount ( PANEL_SPRITEp psp )
{
    PLAYERp pp = psp->PlayerP;
    pp->nukevochandle =
        PlaySound ( DIGI_NUKECDOWN, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
}
...
pMicroReady ( PANEL_SPRITEp psp )
{
    PLAYERp pp = psp->PlayerP;
    pp->nukevochandle =
        PlaySound ( DIGI_NUKEREADY, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
    pp->NukeInitialized = TRUE;
}
...
InitWeaponHeart ( PLAYERp pp )
{
    ...
    pp->WpnUziType = 2; // Make uzi's go away!
    RetractCurWpn ( pp );
    PlaySound ( DIGI_HEARTBEAT, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan | v3df_doppler ) );
    // Set up the newp Weapon variables
...
pHeartAttack ( PANEL_SPRITEp psp )
{
    ...
    // CTW MODIFICATION END
    PlaySound ( DIGI_HEARTFIRE, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
    
    if ( RANDOM_RANGE ( 1000 ) > 800 )
    {
        PlayerSound ( DIGI_JG9009, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), pp );
    }
...
{
    PLAYERp pp = psp->PlayerP;
    PlaySound ( DIGI_MINE_UP, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow ) );
}
...
InitChops ( PLAYERp pp )
{
    ...
    {
        PlayerSound ( DIGI_MRFLY, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), psp->PlayerP );
    }
}
...
{
    SHORT rnd_rng;
    PlaySound ( DIGI_CHOP_CLICK, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_none ) );
    rnd_rng = RANDOM_RANGE ( 1000 );
    
    if ( rnd_rng > 950 )
    {
        PlayerSound ( DIGI_SEARCHWALL, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), psp->PlayerP );
    }
    
    else if ( rnd_rng > 900 )
    {
        PlayerSound ( DIGI_EVADEFOREVER, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), psp->PlayerP );
    }
    
    else if ( rnd_rng > 800 )
    {
        PlayerSound ( DIGI_SHISEISI, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), psp->PlayerP );
    }
}
...
pChopsRetract ( PANEL_SPRITEp psp )
{
    ...
    if ( psp->y >= CHOPS_YOFF + tilesiz[picnum].y )
    {
        if ( RANDOM_RANGE ( 1000 ) > 800 )
        {
            PlayerSound ( DIGI_GETTINGSTIFF, &psp->PlayerP->posx, &psp->PlayerP->posy, &psp->PlayerP->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), psp->PlayerP );
        }
...
InsertPanelSprite ( PLAYERp pp, PANEL_SPRITEp psp )
{
    ...
    // if newp pri is <= first pri in list, insert at front
    ...
    // search for first pri in list thats less than the newp pri
    TRAVERSE ( &pp->PanelSpriteList, cur, nxt )
    {
        ...
        if ( ( ( PANEL_SPRITEp ) psp )->priority >= ( ( PANEL_SPRITEp ) cur )->priority && ( ( PANEL_SPRITEp ) psp )->priority <= ( ( PANEL_SPRITEp ) cur )->Next->priority )
        {
            INSERT ( cur, psp );
            return;
        }
...
{
    OrgTileP psp = NULL, next_psp = NULL;
...
pDisplaySprites ( PLAYERp pp )
{
    ...
    long flags;
    long x1, y1, x2, y2;
    TRAVERSE ( &pp->PanelSpriteList, psp2, next )
    {
        ASSERT ( ValidPtr ( psp ) );
        PANEL_SPRITEp psp = ( PANEL_SPRITEp ) psp2;
        ang = ( ( PANEL_SPRITEp ) psp )->rotate_ang;
        KenFlags = 0;
        shade = 0;
        flags = 0;
        x = psp->x;
...
{
    OrgTileP psp = NULL, next = NULL;
    TRAVERSE ( &pp->PanelSpriteList, psp, next )
    {
        ASSERT ( ValidPtr ( psp ) );
        
        // force kill before showing again
        if ( TEST ( ( ( PANEL_SPRITEp ) psp )->flags, PANF_KILL_AFTER_SHOW ) )
        {
            pKillSprite ( ( PANEL_SPRITEp ) psp );
        }
    }
...
pSpriteControl ( PLAYERp pp )
{
    ...
    TRAVERSE ( &pp->PanelSpriteList, psp, next )
    {
        ...
        PRODUCTION_ASSERT ( ( PANEL_SPRITEp ) psp );
        ASSERT ( ValidPtr ( ( PANEL_SPRITEp ) psp ) );
        ASSERT ( ( ULONG ) ( PANEL_SPRITEp ) psp->Next != 0xCCCCCCCC );
        
        if ( ( ( PANEL_SPRITEp ) psp )->State )
        {
            ...
            // something
            if ( ( ( PANEL_SPRITEp ) psp )->PanelSpriteFunc )
            {
                ( * ( ( PANEL_SPRITEp ) psp )->PanelSpriteFunc ) ( ( ( PANEL_SPRITEp ) psp ) );
            }
    }
}
