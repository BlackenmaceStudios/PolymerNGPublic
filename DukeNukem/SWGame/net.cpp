...
long netgetpacket ( long *ind, char *buf )
{
    ...
    long len;
    PACKET_PROXYp prx;
    len = getpacket ( ( int * ) ind, ( unsigned char * ) buf );
...
InitTimingVars ( VOID )
{
    extern int32_t lastUpdate; // CD tic counter
    PlayClock = 0;
    // resettiming();
    totalsynctics = 0;
            ...
            case PACKET_TYPE_NAME_CHANGE:
                {
                    ...
                    pp = Player + otherconnectindex;
                    p = ( PACKET_NAME_CHANGEp ) &packbuf[0];
