...
TrackClonePoint ( short SpriteNum )
{
    ...
    short newp;
    newp = COVERinsertsprite ( sp->sectnum, sp->statnum );
    ASSERT ( newp != -1 );
    np = &sprite[newp];
    np->cstat = np->extra = 0;
    np->x = sp->x;
...
TrackSetup ( VOID )
{
    ...
    for ( ndx = 0; ndx < MAX_TRACKS; ndx++ )
    {
        if ( headspritestat[STAT_TRACK + ndx] == -1 )
        {
            ...
            Track[ndx].TrackPoint = ( TRACK_POINTp ) ( sizeof ( TRACK_POINT ) * 1, 1 );
            continue;
        }
        ...
        newp = ( TRACK_POINTp ) CallocMem ( size, 1 );
        memcpy ( newp, Track[ndx].TrackPoint, size );
        FreeMem ( Track[ndx].TrackPoint );
...
SetupSectorObject ( short sectnum, short tag )
{
    ...
    switch ( tag % 5 )
    {
        case TAG_OBJECT_CENTER - 500:
            ...
            newp = SpawnSprite ( STAT_SO_SP_CHILD, 0, NULL, sectnum,
                                 sop->xmid, sop->ymid, sop->zmid, 0, 0 );
            sop->sp_child = &sprite[newp];
            u = User[newp];
            u->sop_parent = sop;
...
MovePoints ( SECTOR_OBJECTp sop, short delta_ang, long nx, long ny )
{
    ...
    for ( i = 0; sop->sp_num[i] != -1; i++ )
    {
        ...
        if ( TEST ( u->Flags, SPR_ON_SO_SECTOR ) )
        {
            ...
            {
                rotatepoint ( sop->xmid, sop->ymid, ( long ) sp->x, ( long ) sp->y, ( int16_t ) - delta_ang, ( long * ) &sp->x, ( long * ) &sp->y );
        ...
        else
        {
            if ( !TEST ( sop->flags, SOBJ_DONT_ROTATE ) )
            {
                ...
                rotatepoint ( sop->xmid, sop->ymid, sp->x, sp->y, delta_ang, ( long * ) &sp->x, ( long * ) &sp->y );
                sp->ang = NORM_ANGLE ( sp->ang + delta_ang );
            }
...
RefreshPoints ( SECTOR_OBJECTp sop, long nx, long ny, BOOL dynamic )
{
    ...
    {
        // angle traveling + the newp spin angle all offset from the original
        // angle
...
MoveZ ( SECTOR_OBJECTp sop )
{
    ...
    if ( TEST ( sop->flags, SOBJ_ZDOWN ) )
    {
        for ( i = 0, sectp = &sop->sectp[0]; *sectp; sectp++, i++ )
        {
            AnimSet ( ( long * ) & ( *sectp )->floorz, sop->zorig_floor[i] + sop->z_tgt, sop->z_rate );
        }
...
void CallbackSOsink ( ANIMp ap, void *data )
{
    ...
    for ( i = 0; sop->sector[i] != -1; i++ )
    {
        if ( ap->ptr == ( long * ) &sector[sop->sector[i]].floorz )
        {
...
VOID DoTrack ( SECTOR_OBJECTp sop, short locktics, long *nx, long *ny )
{
    ...
    // NOTE: Jittery ride - try newp value out here
    ...
    if ( sop->target_dist < 100 )
    {
        switch ( tpoint->tag_low )
        {
            ...
            case TRACK_SPEED_UP:
                ...
                if ( sop->dir < 0 )
                {
                    // set target to newp slower target
            ...
            case TRACK_SO_SINK:
                {
                    ...
                    for ( i = 0, sectp = &sop->sectp[0]; *sectp; sectp++, i++ )
                    {
                        ...
                        ndx = AnimSet ( ( long * ) & ( *sectp )->floorz, sector[dest_sector].floorz, tpoint->tag_high );
            ...
            case TRACK_SO_FORM_WHIRLPOOL:
                {
                    ...
                    for ( i = 0, sectp = &sop->sectp[0]; *sectp; sectp++, i++ )
                    {
                        ...
                        {
                            AnimSet ( ( long * ) & ( *sectp )->floorz, ( *sectp )->floorz + Z ( sectu->height ), 128 );
        ...
        // calculate a newp angle to the target
        sop->ang_moving = sop->ang_tgt = getangle ( tpoint->x - sop->xmid, tpoint->y - sop->ymid );
        ...
        if ( TEST ( sop->flags, SOBJ_ZDIFF_MODE ) )
        {
            ...
            {
                // churn through sectors setting their newp z values
                for ( i = 0; sop->sector[i] != -1; i++ )
                {
                    AnimSet ( ( long * ) &sector[sop->sector[i]].floorz, dz - ( sector[sop->mid_sector].floorz - sector[sop->sector[i]].floorz ), sop->z_rate );
                }
            }
        }
    }
...
DoAutoTurretObject ( SECTOR_OBJECTp sop )
{
    ...
    // check for newp player if doesn't have a target or time limit expired
