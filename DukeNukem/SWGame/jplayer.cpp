...
void operatefta ( void )
{
    ...
    if ( MessageInputMode )
    {
        j -= 6;    // Bump all lines up one to make room for newp line
    }
...
void operateconfta ( void )
{
    ...
    if ( ConInputMode ) { j -= 6; } // Bump all lines up one to make room for newp line
...
void computergetinput ( long snum, SW_PACKET *syn )
{
    ...
    // Pick a newp target player if goal is dead or target is itself
