...
char UserMapName[80] = "$bullet.map", buffer[80], ch;
char LevelName[20];
...
CacheAlloc ( void **ptr, long size, char *lock_byte )
{
    if ( *ptr == NULL )
    {
        *lock_byte = CACHE_LOCK_START;
        allocache ( ( intptr_t * ) ptr, size, lock_byte );
    }
...
LoadLevel ( char *filename )
{
    int pos;
    vec3_t position;
    position.x = Player[0].posx;
    position.y = Player[0].posy;
    position.z = Player[0].posz;
    
    if ( loadboard ( filename, SW_SHAREWARE ? 1 : 0, &position, &Player[0].pang, &Player[0].cursectnum ) == -1 )
    {
        TerminateGame();
        #ifdef RENDERTYPEWIN
        {
    ...
    Player[0].posx = position.x;
    Player[0].posy = position.y;
    Player[0].posz = position.z;
}
...
{
    extern int32_t ScreenMode, ScreenWidth, ScreenHeight, ScreenBPP;
    int result;
    char ch;
...
void COVERsetbrightness ( int bright, char *pal )
{
    ...
    setbasepal ( 0, ( const uint8_t * ) &pal[0] );
    setbrightness ( bright, 0, 0 );
    // jmarshall end
}
...
LogoLevel ( VOID )
{
    ...
    if ( ( fin = kopen4load ( "3drealms.pal", 0 ) ) != -1 )
    {
        ...
        kclose ( fin );
        // jmarshall - palette
        setbasepal ( 25, ( const uint8_t * ) &pal[0] );
        setbrightness ( gs.Brightness, 25, 0 );
        // jmarshall end
    }
    ...
    // jmarshall - palette
    setbasepal ( 0, ( const uint8_t * ) &palette_data[0] );
...
void MoveLoop ( void )
{
    ...
    //While you have newp input packets to process...
    if ( !CommEnabled )
    {
        bufferjitter = 0;
    }
...
ManualPlayerInsert ( PLAYERp pp )
{
    ...
    if ( numplayers < MAX_SW_PLAYERS )
    {
        ...
        // If IsAI = TRUE, newp player will be a bot
...
#define SET_LOC_KEY(loc, sync_num, key_test) SET(loc, ((!!(key_test)) << (sync_num)))
   ...
   // info.dyaw = ( info.dyaw * turn_scale ) >> 8;
    ...
    SET_LOC_KEY ( loc->bits, SK_SPACE_BAR, !!(KEY_PRESSED ( KEYSC_SPACE ) || xBuildInputSystem->ControllerKeyDown(XB_Button_Y) ));
	...
	if ( info.dpitch > 0 )
	{
		aimvel = labs ( ( -info.dpitch * 10) >> 10 );
	}

	else
	{
		aimvel = (info.dpitch * 10) >> 10;
	}

	aimvel = -aimvel;
//	svel += info.dpitch; // *NORMALKEYMOVE;
	vel = -info.dz >> 6;
	...
	angvel += (info.dyaw / 100) * NORMALTURN;
    ...
    SET_LOC_KEY ( loc->bits, SK_SHOOT, (BUTTON ( gamefunc_Fire ) || xBuildInputSystem->ControllerKeyDown(XB_Right_Trigger)));
    // actually snap
    ...
    if ( BUTTON ( gamefunc_Next_Weapon ) || xBuildInputSystem->ControllerKeyDown(XB_Shoulder_Left))
    {
    ...
    if ( BUTTON ( gamefunc_Previous_Weapon ) || xBuildInputSystem->ControllerKeyDown(XB_Shoulder_Right) )
    {
        USERp u = User[pp->PlayerSprite];
    ...
    SET_LOC_KEY ( loc->bits, SK_OPERATE, BUTTON ( gamefunc_Open ) || xBuildInputSystem->ControllerKeyDown(XB_Button_Y)) ;
    SET_LOC_KEY ( loc->bits, SK_JUMP, (BUTTON ( gamefunc_Jump ) || xBuildInputSystem->ControllerKeyDown(XB_Button_A) ));
    SET_LOC_KEY ( loc->bits, SK_CRAWL, BUTTON ( gamefunc_Crouch ) || xBuildInputSystem->ControllerKeyDown(XB_Button_B));
