...
void CON_StoreArg ( BYTEp userarg )
{
    ...
    {
        strcpy ( &user_args[con_argnum][0], ( char * ) userarg );
...
BOOL CON_CheckParm ( BYTEp userarg )
{
    ...
    {
        if ( !strcmp ( &user_args[i][0], ( char * ) userarg ) )
        {
...
BOOL CON_AddCommand ( BYTEp command, /*BOOL*/void ( *function ) ( void ) )
{
    if ( command != NULL && function != NULL && numcommands < MAX_CONSOLE_COMMANDS )
    {
        ...
        commandlist[numcommands].command = ( char * ) command;
...
void CON_InitConsole ( void )
{
    ...
    {
        if ( !CON_AddCommand ( ( BYTEp ) i->command, i->function ) )
        {
...
void CON_LoadSetup ( void )
{
    ...
    CON_ConMessage("Loaded newp config file.");
    */
