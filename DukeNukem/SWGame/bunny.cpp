...
#define MAX_BUNNYS 1
    ...
    {
        newp = COVERinsertsprite ( wp->sectnum, STAT_DEFAULT );
        np = &sprite[newp];
        ...
        else
        {
            ...
            if ( RANDOM_RANGE ( 1000 ) > 500 )
            {
                ...
                {
                    DoActorDie ( Weapon, newp );
                }
            }
        }
        
        nu->ShellNum = 0; // Not Pregnant right now
        NewStateGroup ( newp, nu->ActorActionSet->Jump );
        nu->ActorActionFunc = DoActorMoveJump;
        DoActorSetSpeed ( newp, FAST_SPEED );
        PickJumpMaxSpeed ( newp, -600 );
...
int BunnyHatch2 ( short Weapon )
{
    ...
    if ( TEST_BOOL3 ( wp ) )
    {
        PickJumpMaxSpeed ( newp, -600 - RANDOM_RANGE ( 600 ) );
