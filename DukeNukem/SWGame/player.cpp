            ...
            TAG 3 = Type
                    ...
                    TAG 4 = angle
                            TAG 5 to 8 = random match locations
                                         ...
                                         if ( ( sp_warp = Warp ( ( int32_t * ) &pp->posx, ( int32_t * ) &pp->posy, ( int32_t * ) &pp->posz, &pp->cursectnum ) ) == NULL )
    {
        return;
    }
...
#define TURN_SHIFT 2
    ...
    if ( TEST ( pp->Flags, PF_TURN_180 ) )
    {
        ...
        // get newp delta to see how close we are
        delta_ang = GetDeltaAngle ( pp->turn180_target, pp->pang );
...
PlayerAutoLook ( PLAYERp pp )
{
    ...
    if ( !TEST ( pp->Flags, PF_FLYING | PF_SWIMMING | PF_DIVING | PF_CLIMBING | PF_JUMPING | PF_FALLING ) )
    {
        if ( TEST ( sector[pp->cursectnum].floorstat, FLOOR_STAT_SLOPE ) ) // If the floor is sloped
        {
            ...
            if ( tempsect >= 0 )            // If the newp point is inside a valid
                // sector...
            {
                // Get the floorz as if the newp (x,y) point was still in
                // your sector
...
DoPlayerMove ( PLAYERp pp )
{
    ...
    if ( WarpPlane ( ( int32_t * ) &pp->posx, ( int32_t * ) &pp->posy, ( int32_t * ) &pp->posz, &pp->cursectnum ) )
    {
    ...
    for ( i = 0; i < synctics; i++ )
    {
        ...
        if ( PlayerFloorHit ( pp, pp->loz - PLAYER_HEIGHT + recoil_amt ) )
        {
            ...
            else
            {
                ...
                {
                    PlaySound ( DIGI_HITGROUND, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
                }
            }
            ...
            {
                PlayerSound ( DIGI_PLAYERPAIN2, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), pp );
...
#define ADJ_AMT 8
    ...
    if ( LadderUpdate )
    {
        SPRITEp lsp;
        long nx, ny;
        // constantly look for newp ladder sector because of warping at any time
...
DoPlayerStopDiveNoWarp ( PLAYERp pp )
{
    ...
    // stop diving no warp
    PlayerSound ( DIGI_SURFACE, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_dontpan | v3df_follow | v3df_doppler ), pp );
    pp->bob_amt = 0;
...
DoPlayerDive ( PLAYERp pp )
{
    ...
    if ( ( pp->DiveTics -= synctics ) < 0 )
    {
        if ( ( pp->DiveDamageTics -= synctics ) < 0 )
        {
            ...
            PlayerSound ( DIGI_WANGDROWNING, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_dontpan | v3df_follow ), pp );
...
DoPlayerBeginOperate ( PLAYERp pp )
{
    ...
    if ( sop->max_damage != -9999 && sop->max_damage <= 0 )
    {
        ...
        {
            PlayerSound ( DIGI_USEBROKENVEHICLE, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), pp );
            return;
        }
    }
        ...
        if ( TEST_SYNC_KEY ( pp, SK_SHOOT ) )
        {
            if ( FLAG_KEY_PRESSED ( pp, SK_SHOOT ) )
            {
                ...
                if ( MoveSkip4 == 0 )
                {
                    ...
                    {
                        PlayerSound ( DIGI_DHVOMIT, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_dontpan | v3df_follow ), pp );
                    }
                }
...
InitMultiPlayerInfo ( VOID )
{
    ...
    for ( stat = 0; stat < SIZ ( MultiStatList ); stat++ )
    {
        ...
        {
            // if start position is physically set then don't spawn a newp one
