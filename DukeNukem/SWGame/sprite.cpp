...
KillSprite ( SHORT SpriteNum )
{
    ...
    if ( u )
    {
        ...
        // any Anims attached
        AnimDelete ( ( long * ) &u->sz );
        AnimDelete ( ( long * ) &sp->z );
...
{
    int i;
    short anim_type = TEST ( picanm[picnum].rawbits, TILE_ANIM_TYPE );
    short num;
    ...
    */
    RESET ( picanm[picnum].rawbits, TILE_ANIM_TYPE );
}
...
ActorTestSpawn ( SPRITEp sp )
{
    if ( sp->statnum == STAT_DEFAULT && sp->lotag == TAG_SPAWN_ACTOR )
    {
        ...
        newp = COVERinsertsprite ( sp->sectnum, STAT_DEFAULT );
        memcpy ( &sprite[newp], sp, sizeof ( SPRITE ) );
        change_sprite_stat ( newp, STAT_SPAWN_TRIGGER );
        RESET ( sprite[newp].cstat, CSTAT_SPRITE_BLOCK | CSTAT_SPRITE_BLOCK_HITSCAN );
        return ( FALSE );
    }
                            ...
                            case SECT_ROTATOR:
                                {
                                    ...
                                    u->rotator->origx = ( long * ) CallocMem ( sizeof ( u->rotator->origx ) * wallcount, 1 );
                                    u->rotator->origy = ( long * ) CallocMem ( sizeof ( u->rotator->origy ) * wallcount, 1 );
                            ...
                            case LIGHTING:
                                {
                                    ...
                                    wall_shade = u->WallShade = ( CHARp ) CallocMem ( u->WallCount * sizeof ( *u->WallShade ), 1 );
            ...
            KeyMain:
                {
                    ...
                    RESET ( picanm[sp->picnum].rawbits, TILE_ANIM_TYPE );
                    RESET ( picanm[sp->picnum + 1].rawbits, TILE_ANIM_TYPE );
                    change_sprite_stat ( SpriteNum, STAT_ITEM );
                    RESET ( sp->cstat, CSTAT_SPRITE_BLOCK | CSTAT_SPRITE_BLOCK_HITSCAN );
                    u->Radius = 500;
...
int ActorCoughItem ( short SpriteNum )
{
    ...
    switch ( u->ID )
    {
        case SAILORGIRL_R0:
            ASSERT ( sp->sectnum >= 0 );
            newp = COVERinsertsprite ( sp->sectnum, STAT_SPAWN_ITEMS );
            ASSERT ( newp >= 0 );
            np = &sprite[newp];
            np->cstat = np->extra = 0;
            np->x = sp->x;
...
NewStateGroup ( short SpriteNum, STATEp StateGroup[] )
{
    ...
    // art file
    RESET ( picanm[sprite[SpriteNum].picnum].rawbits, TILE_ANIM_TYPE );
    return 0;
}
                 ...
                 LONGp ceilz, LONGp ceilhit, LONGp florz, LONGp florhit )
{
    ...
    getzsofslope ( sectnum, x, y, ( int32_t * ) ceilz, ( int32_t * ) florz );
    ...
    *florhit = sectnum + 16384;
    ...
    for ( j = headspritesect[sectnum]; j >= 0; j = nextspritesect[j] )
    {
        ...
        tilenum = spr->picnum;
        xoff = ( long ) ( ( signed char ) ( ( picanm[tilenum].rawbits >> 8 ) & 255 ) ) + ( ( long ) spr->xoffset );
        yoff = ( long ) ( ( signed char ) ( ( picanm[tilenum].rawbits >> 16 ) & 255 ) ) + ( ( long ) spr->yoffset );
...
DoActorZrange ( short SpriteNum )
{
    ...
    RESET ( sp->cstat, CSTAT_SPRITE_BLOCK );
    FAFgetzrange ( sp->x, sp->y, sp->z - DIV2 ( SPRITEp_SIZE_Z ( sp ) ), sp->sectnum, ( LONGp ) &u->hiz, &ceilhit, ( LONGp ) &u->loz, &florhit, ( ( ( long ) sp->clipdist ) << 2 ) - GETZRANGE_CLIP_ADJ, CLIPMASK_ACTOR );
...
KillGet ( short SpriteNum )
{
    ...
    switch ( gNet.MultiGameType )
    {
        ...
        case MULTI_GAME_AI_BOTS:
            ...
            newp = SpawnSprite ( STAT_ITEM, Red_COIN, s_RedCoin, sp->sectnum,
                                 sp->x, sp->y, sp->z, 0, 0 );
            np = &sprite[newp];
            nu = User[newp];
            np->shade = -20;
...
void ChoosePlayerGetSound ( PLAYERp pp )
{
    ...
    PlayerSound ( PlayerGetItemVocs[choose_snd], &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ), pp );
}
        ...
        switch ( u->ID )
        {
            ...
            case ICON_UZIFLOOR:
                ...
                if ( TEST ( pp->WpnFlags, BIT ( WPN_UZI ) ) )
                {
                    ...
                    if ( pp == Player + myconnectindex )
                    {
                        PlayerSound ( DIGI_DOUBLEUZI, &pp->posx, &pp->posy, &pp->posz, ( Voc3D_Flags ) ( v3df_dontpan | v3df_follow ), pp );
                    }
                }
...
#define TEXT_SPELL_INFO_LINE 20
            ...
            case ICON_FLAG:
                ...
                if ( sp->hitag == TAG_NORESPAWN_FLAG )
                    newp = SpawnSprite ( STAT_ITEM, ICON_FLAG, s_CarryFlagNoDet, sp->sectnum,
                                         sp->x, sp->y, sp->z, 0, 0 );
                else
                    newp = SpawnSprite ( STAT_ITEM, ICON_FLAG, s_CarryFlag, sp->sectnum,
                                         sp->x, sp->y, sp->z, 0, 0 );
...
move_sprite ( short spritenum, long xchange, long ychange, long zchange, long ceildist, long flordist, ULONG cliptype, long numtics )
{
    ...
    // Must do this if not using the newp
    ...
    clipmoveboxtracenum = 1;
    retval = clipmove ( ( long * ) &spr->x, ( long * ) &spr->y, &daz, &dasectnum,
...
int pushmove_sprite ( short SpriteNum )
{
    ...
    daz = sp->z - u->zclip;
    sectnum = sp->sectnum;
    ret = pushmove ( ( long * ) &sp->x, ( long * ) &sp->y, &daz, &sectnum,
...
move_missile ( short spritenum, long xchange, long ychange, long zchange, long ceildist, long flordist, ULONG cliptype, long numtics )
{
    ...
    clipmoveboxtracenum = 1;
    retval = clipmove ( ( long * ) &sp->x, ( long * ) &sp->y, &daz, &dasectnum,
                        ( ( xchange * numtics ) << 11 ), ( ( ychange * numtics ) << 11 ),
