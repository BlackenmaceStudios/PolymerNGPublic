...
short CopyQuakeSpotToOn ( SPRITEp sp )
{
    short newp;
    SPRITEp np;
    newp = COVERinsertsprite ( sp->sectnum, STAT_QUAKE_SPOT );
    np = &sprite[newp];
    ...
    np->extra = 0;
    np->owner = -1;
    change_sprite_stat ( newp, STAT_QUAKE_ON );
                 ...
                 short tics, short amt, long radius )
{
    ...
    QUAKE_AngAmt ( sp ) = 8;
    QUAKE_PosAmt ( sp ) = 0;
    PlaySound ( DIGI_ERUPTION, &sp->x, &sp->y, &sp->z, ( Voc3D_Flags ) ( v3df_follow | v3df_dontpan ) );
