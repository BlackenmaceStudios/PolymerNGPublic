...
NewCoolg ( short SpriteNum )
{
    ...
    ANIMATOR DoActorDecide;
    short newp;
    newp = SpawnSprite ( STAT_ENEMY, COOLG_RUN_R0, &s_CoolgBirth[0], sp->sectnum, sp->x, sp->y, sp->z, sp->ang, 50 );
    nu = User[newp];
    np = &sprite[newp];
    ChangeState ( newp, &s_CoolgBirth[0] );
...
DoCoolgBirth ( short newp )
{
    ...
    u->Health = HEALTH_COOLIE_GHOST;
    u->Attrib = &CoolgAttrib;
    DoActorSetSpeed ( newp, NORM_SPEED );
    ChangeState ( newp, s_CoolgRun[0] );
    u->StateEnd = s_CoolgDie;
    u->Rot = sg_CoolgRun;
    EnemyDefaults ( newp, &CoolgActionSet, &CoolgPersonality );
    // special case
    TotalKillable--;
    SET ( u->Flags, SPR_NO_SCAREDZ | SPR_XFLIP_TOGGLE );
    CoolgCommon ( newp );
