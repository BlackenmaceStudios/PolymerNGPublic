        ...
        switch ( MNU_InputString ( MessageInputString, 80 ) )
        {
            ...
            case FALSE: // Input finished (RETURN)
                ...
                else
                {
                    ...
                    {
                        if ( !Bstrcasecmp ( ( char * ) gs.Password, MessageInputString ) )
                        {
                            passwordvalid = TRUE;
                    ...
                    else
                    {
                        if ( currentmenu->cursor == 1 ) // Is it on the password line?
                        {
                            strcpy ( ( char * ) gs.Password, MessageInputString );
                            passwordvalid = FALSE;
                        }
                    }
...
MNU_InitMenus ( void )
{
    ...
    slidersettings[sldr_bordertile] = gs.BorderTile;
    {
        ...
        i = checkvideomode ( &newx, &newy, bpp, fullscreen, 0 );
    ...
    // Distinguish between Single or Multiplay for newp game menu types
    if ( numplayers > 1 )
    {
...
MNU_DoItem ( void )
{
    ...
    switch ( item->type )
    {
        ...
        case mt_layer:
            ...
            MNU_DownLevel ( ( MenuGroup * ) item->child );
            break;
...
MNU_NextItem ( void )
{
    ...
    flag = ( MenuFlags ) currentmenu->items[currentmenu->cursor + 1].flags;
    ...
    flag = ( MenuFlags ) currentmenu->items[currentmenu->cursor].flags;
...
SetFadeAmt ( PLAYERp pp, short damage, unsigned char startcolor )
{
    ...
    if ( pp == Player + screenpeek )
    {
        ...
        if ( pp->FadeAmt <= 0 )
        {
            GetPaletteFromVESA ( ( char * ) &ppalette[screenpeek][0] );
        }
    }
    ...
    GetPaletteFromVESA ( ( char * ) pp->temp_pal );
    ...
    if ( pp == Player + screenpeek )
    {
        if ( getrendermode() < 3 ) { set_pal ( ( char * ) pp->temp_pal ); }
