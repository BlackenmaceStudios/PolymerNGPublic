...
SpawnZombie ( PLAYERp pp, short Weapon )
{
    ...
    //Zombies++;
    newp = SpawnSprite ( STAT_ENEMY, ZOMBIE_RUN_R0, s_ZombieRun[0], pp->cursectnum, pp->posx, pp->posy, pp->posz, pp->pang, 0 );
    np = &sprite[newp];
    nu = User[newp];
...
SpawnZombie2 ( short Weapon )
{
    ...
    //Zombies++;
    newp = SpawnSprite ( STAT_ENEMY, ZOMBIE_RUN_R0, s_ZombieRun[0], sp->sectnum, sp->x, sp->y, sp->z, sp->ang, 0 );
    np = &sprite[newp];
    nu = User[newp];
    nu->Counter3 = 0;
    np->owner = owner;
