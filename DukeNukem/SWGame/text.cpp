...
pClearTextLineID ( PLAYERp pp, short id, long y, short pri )
{
    ...
    TRAVERSE ( &pp->PanelSpriteList, psp, next )
    {
        ...
        if ( ( ( PANEL_SPRITEp ) psp )->ID == id && ( ( PANEL_SPRITEp ) psp )->y == y && ( ( PANEL_SPRITEp ) psp )->priority == pri )
        {
            SetRedrawScreen ( ( ( PANEL_SPRITEp ) psp )->PlayerP );
            //SET(psp->flags, PANF_INVISIBLE);
            pSetSuicide ( ( ( PANEL_SPRITEp ) psp ) );
        }
    }
    return ( NULL );
}
...
pClearSpriteXY ( PLAYERp pp, short x, short y )
{
    ...
    TRAVERSE ( &pp->PanelSpriteList, psp, next )
    {
        if ( ( ( PANEL_SPRITEp ) psp )->x == x && ( ( PANEL_SPRITEp ) psp )->y == y )
        {
            pSetSuicide ( ( ( PANEL_SPRITEp ) psp ) );
        }
    }
