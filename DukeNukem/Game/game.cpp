...
// jmarshall
//                        if (getrendermode() >= REND_POLYMOST && usemodels && md_tilehasmodel(s->picnum,t->pal) >= 0 && !(spriteext[i].flags&SPREXT_NOTMD))
						if (usemodels && md_tilehasmodel(s->picnum, t->pal) >= 0 && !(spriteext[i].flags&SPREXT_NOTMD))
...
void G_UpdateAppTitle(void)
{
    if (g_gameNamePtr)
    {
        Bsprintf(tempbuf, "%s (" APPNAME " Build " __DATE__ " " __TIME__ ") D3D12 POLYMERNG", g_gameNamePtr);
...
//#endif
	...
	switch (currentDukeExpansion)
	{
		case DUKE_EXPANSION_ATOMIC:
			initprintf("loading as atomic edition\n");
		break;
		case DUKE_EXPANSION_DC:
			initprintf("loading as dc\n");
			addsearchpath("Assets/DukeData/Addons/dc/");
			G_AddGroup("dukedc.grp");
			break;

		case DUKE_EXPANSION_VACATION:
			initprintf("loading as vacation\n");
			addsearchpath("Assets/DukeData/Addons/vacation/");
			G_AddGroup("vacation.grp");
			break;

		case DUKE_EXPANSION_NW:
			initprintf("loading as nw\n");
			addsearchpath("Assets/DukeData/Addons/nw/");
			G_AddGroup("nwinter.grp");
			G_AddCon("nwinter.con");
			break;

		case DUKE_MOD_BLOODCM:
			initprintf("loading as bloodcm\n");
			addsearchpath("Assets/DukeData/mods/bloodcm/");
			G_AddGroup("BloodCM.grp");
			G_AddCon("blcons/bloodcm.con");
			break;
	};
