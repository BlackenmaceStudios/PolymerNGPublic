...
//     OSD_Printf("running: %d\n", running);
    ...
    if (BUTTON(gamefunc_Previous_Weapon) || xBuildInputSystem->ControllerKeyDown(XB_Shoulder_Left) || (BUTTON(gamefunc_Dpad_Select)  && in.fvel < 0))
        j = 11;
    if (BUTTON(gamefunc_Next_Weapon) || xBuildInputSystem->ControllerKeyDown(XB_Shoulder_Right) || (BUTTON(gamefunc_Dpad_Select) && in.fvel > 0))
        j = 12;

    if ((BUTTON(gamefunc_Jump) || xBuildInputSystem->ControllerKeyDown(XB_Button_B)) && p->on_ground)
        g_emuJumpTics = 4;

    loc.bits = ((g_emuJumpTics > 0 || BUTTON(gamefunc_Jump) || xBuildInputSystem->ControllerKeyDown(XB_Button_B))<<SK_JUMP) ;

    if (g_emuJumpTics > 0)
        g_emuJumpTics--;

    loc.bits |=   (BUTTON(gamefunc_Crouch) || xBuildInputSystem->ControllerKeyDown(XB_Button_Y)) <<SK_CROUCH;
    ...
    loc.bits |= (BUTTON(gamefunc_Inventory_Left) || xBuildInputSystem->ControllerKeyDown(XB_Button_DPAD_Left) || (BUTTON(gamefunc_Dpad_Select) && (in.svel > 0 || in.avel < 0))) <<SK_INV_LEFT;
    ...
    loc.bits |= (BUTTON(gamefunc_Inventory_Right) || (BUTTON(gamefunc_Dpad_Select) || xBuildInputSystem->ControllerKeyDown(XB_Button_DPAD_Right) && (in.svel < 0 || in.avel > 0))) <<SK_INV_RIGHT;
    loc.bits |=   BUTTON(gamefunc_TurnAround)<<SK_TURNAROUND;
    loc.bits |=  ((BUTTON(gamefunc_Open) || xBuildInputSystem->ControllerKeyDown(XB_Button_A) )<<SK_OPEN) ;
    loc.bits |=   (BUTTON(gamefunc_Inventory) || xBuildInputSystem->ControllerKeyDown(XB_Button_X)) <<SK_INVENTORY;
