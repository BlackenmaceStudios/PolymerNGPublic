...
static void CONTROL_GetMouseDelta(void)
{
    ...
    CONTROL_MouseAxes[0].analog = (int32_t)(x * CONTROL_MouseSensitivity * 2.0f);
    CONTROL_MouseAxes[1].analog = (int32_t)((y * CONTROL_MouseSensitivity) * 2.0f);
}
#endif
...
// Controller support - jmarshall - very hacky.
	XHandleControllerMovement(&info->dx, &info->dz);
...
//    ticrate = ticspersecond;
    ...
    CONTROL_MousePresent = CONTROL_MouseEnabled    = TRUE;
