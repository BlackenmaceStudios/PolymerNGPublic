...
FILE *file_opend = NULL;
static int32_t kopen4load(const char *name, int type)
{
	file_opend = fopen(name, "rb");
	if (file_opend == NULL)
		return 0;
...
static int32_t kread(int32_t handle, void *buffer, int32_t leng)
{
	return fread(buffer, leng, 1, file_opend);
}

static int32_t klseek(int32_t handle, int32_t offset, int32_t whence)
{
	return fseek(file_opend, offset, whence);
}

static int32_t ktell(int32_t handle)
{
	return ftell(file_opend);
}

static int32_t kfilelength(int32_t handle)
{
	int32_t startposition = ktell(handle);
	fseek(file_opend, 0, SEEK_END);
	int32_t len = ktell(handle);
	fseek(file_opend, startposition, SEEK_SET);
	return len;
}

static void kclose(int32_t handle)
{
	fclose(file_opend);
	file_opend = NULL;
}
